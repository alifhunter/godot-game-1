...
- Version control:
  ...
  - recent committed checkpoints include:
    ...
    - latest checkpoint message: `Count Twooter activity without feed rebuild`
  - after checkpoint commits, `git status --short` should be clean except ignored local `logs/` output

## Latest Session Snapshot
- Most recent work focused on the core daily loop, Academy authoring pipeline, `Advance Day` visible-refresh performance, Daily Activity cache cost, Dashboard cache cost, deferred save durability, and shareholder-only corporate meeting attendance.
- Daily loop status:
...
- Performance status:
  ...
  - Latest normal-play perf scene summary: `open_network=33.8ms`, `advance_network_open_recap_ready=678.08ms`, `advance_network_open=988.4ms`, `advance_desktop_only_recap_ready=670.46ms`, `advance_desktop_only=880.6ms`, `open_stock=65.05ms`, `advance_stock_open_recap_ready=810.04ms`, `advance_stock_open=1183.37ms`, `open_news=241.03ms`, `open_network_with_news=44.46ms`, `advance_news_network_open_recap_ready=829.65ms`, `advance_news_network_open=1319.72ms`, `flush_pending_save=233.43ms`, `local_save_bytes=2010467`.
  - Current UI-button Advance Day logs show `request_save` around `0.05-0.16ms` in the guarded backend path instead of `save_active_run`; the post-recap flush logs separately around `196-305ms`, and direct `GameManager.advance_day()` still logs immediate `save_active_run` for non-UI callers.
  - Current backend Advance Day logs show `build_dashboard_event_cache` around `16-20ms`, `build_daily_activity_cache` around `1ms`, `build_daily_activity_cache:social_count` around `0.5-0.7ms`, `build_daily_summary` around `60-82ms`, `build_news_snapshot` around `43-95ms`, `emit_price_formed` around `56-67ms`, and `emit_summary_ready` around `36-49ms`; Daily Recap snapshot construction inside `summary_ready` is around `3-5ms`, Network activity count is near-zero, and deferred app redraws still appear after recap/save as per-app logs.
...
- Current verification status:
  - Most recent pass on `2026-04-26`:
    ...
    - quick Godot headless smoke with `--log-file logs\smoke-news-activity.log --scene res://scenes/tests/SmokeTest.tscn -- --smoke-quick --smoke-local-io` passed and printed `SMOKE_QUICK_OK`
    - normal-play perf scene with `--log-file logs\normal_play_perf_news_activity.log --scene res://scenes/tests/NormalPlayPerfTest.tscn -- --smoke-local-io` passed with `NORMAL_PLAY_PERF_OK`
    - latest local headless perf result after count-only Twooter activity and Summary/News split logs: `open_network=33.8ms`, `advance_network_open_recap_ready=678.08ms`, `advance_network_open=988.4ms`, `advance_desktop_only_recap_ready=670.46ms`, `advance_desktop_only=880.6ms`, `open_stock=65.05ms`, `advance_stock_open_recap_ready=810.04ms`, `advance_stock_open=1183.37ms`, `open_news=241.03ms`, `open_network_with_news=44.46ms`, `advance_news_network_open_recap_ready=829.65ms`, `advance_news_network_open=1319.72ms`, `flush_pending_save=233.43ms`, `local_save_bytes=2010467`
    - relevant engine perf logs from the same run: UI-button Advance Day backend uses `request_save` around `0.05-0.16ms` instead of `save_active_run`; post-recap save flush logs separately around `196-305ms`; `build_dashboard_event_cache` roughly `16-20ms`; `build_daily_activity_cache` roughly `1ms`; `build_daily_activity_cache:social_count` roughly `0.5-0.7ms`; `build_daily_summary` roughly `60-82ms`; `build_news_snapshot` roughly `43-95ms`; `build_news_snapshot:company_rows` roughly `16-28ms`; `build_news_snapshot:feed_system` roughly `26-74ms`; `emit_price_formed` roughly `56-67ms`; `_on_summary_ready:daily_recap_snapshot` roughly `3-5ms`; `_on_summary_ready` roughly `36-49ms`
    - this covered Summary and News split perf logs, count-only Twooter current-day activity matching the rendered Twooter post count, shared News/Twooter feed context reuse during Advance Day, deferred UI save flush after Daily Recap visibility, direct `GameManager.advance_day()` immediate flush, queued per-app open-window refreshes, cached Daily Recap activity counts, persisted desktop badge counts, cached Dashboard event snapshots, shareholder-only `RUPS` / `RUPSLB` attendance rejection for zero-position players, blocked zero-position interactive `RUPSLB` overlay entry, and the shareholder-owned interactive `RUPSLB` vote flow
