# Gorengan: Stock Trading Simulator Handoff
...
- Player-facing project name changed to `Gorengan: Stock Trading Simulator`. BuildInfo, export metadata/path, README, Main Menu title, and desktop title now use the new name. Godot `config/name` intentionally uses the Windows-safe `Gorengan Stock Trading Simulator` because `:` can break `user://` save paths on Windows; internal save filenames and format ids still keep legacy `daytrader` strings for compatibility. Verification for this rename pass: `git diff --check` passed, Godot headless project load passed, and quick smoke passed with `SMOKE_QUICK_OK normal_equity=94009661.38 days=3`; only the known Windows root-certificate warning appeared after success.
...
- UI click audio is implemented. `autoloads/UiAudio.gd` is registered as a singleton and automatically connects to every `BaseButton`, `ItemList`, and `OptionButton` as it appears, playing the MP3 at `assets/sound/ElevenLabs_Button_press_sound_of_a_gaming_mouse,_high_precision.mp3` through a small four-player pool so rapid clicks do not cut each other off.
- The click sound loader reads the MP3 bytes into `AudioStreamMP3` instead of preloading the raw file, because Godot headless rejected raw `.mp3` preload before import. Verification for this pass: `git diff --check` passed, Godot headless project load passed, and quick smoke passed with `SMOKE_QUICK_OK normal_equity=94007461.68 days=3`; only the known Windows root-certificate warning appeared after success.
...
`Read PROJECT_HANDOFF.md first, then continue Gorengan: Stock Trading Simulator from there.`
