...
# Life-sim tuning values live in systems/LifeStateSystem.gd; these aliases keep
# the existing RunState.X access paths working for external callers.
const LIFE_STATE_SYSTEM := preload("res://systems/LifeStateSystem.gd")
const CASH_STRESS_GRACE_TRADING_DAYS := LIFE_STATE_SYSTEM.CASH_STRESS_GRACE_TRADING_DAYS
const EMERGENCY_LOAN_PAYMENT_COUNT := LIFE_STATE_SYSTEM.EMERGENCY_LOAN_PAYMENT_COUNT
const EMERGENCY_LOAN_REPAYMENT_MULTIPLIER := LIFE_STATE_SYSTEM.EMERGENCY_LOAN_REPAYMENT_MULTIPLIER
const MAX_LIFE_FINANCE_HISTORY := LIFE_STATE_SYSTEM.MAX_LIFE_FINANCE_HISTORY
const LIFE_BASICS_TIER_IDS := LIFE_STATE_SYSTEM.LIFE_BASICS_TIER_IDS
const LIFE_DEFAULT_BASICS_TIER_ID := LIFE_STATE_SYSTEM.LIFE_DEFAULT_BASICS_TIER_ID
const LIFE_DEFAULT_STRESS_VALUE := LIFE_STATE_SYSTEM.LIFE_DEFAULT_STRESS_VALUE
const LIFE_DEFAULT_HAPPINESS_VALUE := LIFE_STATE_SYSTEM.LIFE_DEFAULT_HAPPINESS_VALUE
const LIFE_BURNOUT_WARNING_TRADING_DAYS := LIFE_STATE_SYSTEM.LIFE_BURNOUT_WARNING_TRADING_DAYS
const LIFE_HOSPITAL_TRADING_DAYS := LIFE_STATE_SYSTEM.LIFE_HOSPITAL_TRADING_DAYS
const LIFE_LEGAL_HOLD_MAX_TRADING_DAYS := LIFE_STATE_SYSTEM.LIFE_LEGAL_HOLD_MAX_TRADING_DAYS
const LIFE_HOSPITAL_RECOVERY_STRESS := 45.0
const LIFE_HOSPITAL_RECOVERY_HAPPINESS := 45.0
const LEGACY_BODETABEK_LIFE_LOCATION_IDS := LIFE_STATE_SYSTEM.LEGACY_BODETABEK_LIFE_LOCATION_IDS

# Quarterly filing simulation weights (used by _build_quarterly_filing_payload).
# QoQ growth target = base*BASE + macro*MACRO + micro*MICRO + market*MARKET + noise.
const FILING_QOQ_BASE_GROWTH_WEIGHT := 0.22
const FILING_QOQ_MACRO_WEIGHT := 0.46
const FILING_QOQ_MICRO_WEIGHT := 0.34
const FILING_QOQ_MARKET_WEIGHT := 0.20
const FILING_YOY_MACRO_WEIGHT := 1.85
const FILING_YOY_MICRO_WEIGHT := 1.55
const FILING_YOY_MARKET_WEIGHT := 0.72
# Blend between QoQ-derived and YoY-derived revenue (0 = pure QoQ, 1 = pure YoY).
const FILING_YOY_BLEND_WEIGHT := 0.58
# Margin pressure contributions per generation trait (trait - 0.5 scaled by these).
const FILING_MARGIN_STRENGTH_WEIGHT := 0.080
const FILING_EXECUTION_CONSISTENCY_WEIGHT := 0.052
const FILING_BALANCE_SHEET_WEIGHT := 0.036
...
func load_from_dict(data: Dictionary) -> void:
	...
	thesis_research_tray = _normalize_thesis_research_tray(data.get("thesis_research_tray", {}))
...
func to_save_dict() -> Dictionary:
	...
	return {
		...
		"thesis_research_tray": _normalize_thesis_research_tray(thesis_research_tray),
...
func get_thesis_research_tray() -> Dictionary:
	thesis_research_tray = _normalize_thesis_research_tray(thesis_research_tray)
	return thesis_research_tray.duplicate(true)


func set_thesis_research_tray(next_tray: Dictionary) -> void:
	thesis_research_tray = _normalize_thesis_research_tray(next_tray)


func get_research_evidence(evidence_id: String) -> Dictionary:
	thesis_research_tray = _normalize_thesis_research_tray(thesis_research_tray)
	if evidence_id.is_empty() or not thesis_research_tray.has(evidence_id):
		return {}
	return thesis_research_tray[evidence_id].duplicate(true)
...
func get_life_stress_ap_penalty(life_state: Dictionary = {}) -> int:
	...
	return LifeStateSystem.stress_ap_penalty(source)
...
func get_life_stress_stage(life_state: Dictionary = {}) -> Dictionary:
	...
	if source.is_empty():
		...
		source = _normalize_life_state(source)
	return LifeStateSystem.stress_stage(source)
...
func refresh_cash_stress_state() -> Dictionary:
	...
	LifeStateSystem.update_cash_stress(finance, float(player_portfolio.get("cash", 0.0)), day_index, current_trade_date)
	set_life_finance(finance)
	return get_life_finance()
...
func apply_emergency_loan_proceeds(principal: float, detail: Dictionary = {}) -> Dictionary:
	...
	player_portfolio["cash"] = cash_after
	LifeStateSystem.start_emergency_loan(finance, normalized_principal, detail, day_index, current_trade_date)
	set_life_finance(finance)
	_record_trade(
		"life",
		"life_emergency_loan",
...
func _default_life_state() -> Dictionary:
	return LifeStateSystem.default_life_state(day_index)



func _default_life_legal_state() -> Dictionary:
	return LifeStateSystem.default_life_legal_state()



func _default_life_finance_state() -> Dictionary:
	return LifeStateSystem.default_life_finance_state()



func _normalize_life_state(source_life: Variant) -> Dictionary:
	return LifeStateSystem.normalize_life_state(source_life, day_index, run_seed)



func _normalize_life_location_id(location_id: String, seed_key: String = "") -> String:
	return LifeStateSystem.normalize_life_location_id(location_id, run_seed, seed_key)



func _life_location_label_for_normalized_id(location_id: String, fallback_label: String = "") -> String:
	return LifeStateSystem.life_location_label_for_normalized_id(location_id, fallback_label)



func _sanitize_legacy_life_location_text(text_value: String, location_id: String) -> String:
	return LifeStateSystem.sanitize_legacy_life_location_text(text_value, location_id)



func _normalize_life_properties(source_properties: Variant) -> Array:
	return LifeStateSystem.normalize_life_properties(source_properties, day_index, run_seed)



func _normalize_life_property_value_events(source_events: Variant) -> Array:
	return LifeStateSystem.normalize_life_property_value_events(source_events, day_index)



func _normalize_life_development_leads(source_leads: Variant) -> Array:
	return LifeStateSystem.normalize_life_development_leads(source_leads, day_index, run_seed)
...
func _normalize_life_cars(source_cars: Variant) -> Array:
	return LifeStateSystem.normalize_life_cars(source_cars, day_index)



func _normalize_life_legal_state(source_legal: Variant) -> Dictionary:
	return LifeStateSystem.normalize_life_legal_state(source_legal)



func _normalize_life_finance_state(source_finance: Variant) -> Dictionary:
	return LifeStateSystem.normalize_life_finance_state(source_finance, day_index)



func _normalize_life_loan(source_loan: Variant) -> Dictionary:
	return LifeStateSystem.normalize_life_loan(source_loan, day_index)



func _append_life_finance_history(finance: Dictionary, row: Dictionary) -> void:
	LifeStateSystem.append_life_finance_history(finance, row)



func _default_twooter_social_state() -> Dictionary:
	return TwooterStateSystem.default_social_state(day_index)



func _normalize_twooter_social_state(source_state: Variant) -> Dictionary:
	return TwooterStateSystem.normalize_social_state(source_state, day_index)



func _normalize_twooter_account_state(source_state: Variant) -> Dictionary:
	return TwooterStateSystem.normalize_account_state(source_state)



func _normalize_twooter_post_interaction(source_interaction: Variant) -> Dictionary:
	return TwooterStateSystem.normalize_post_interaction(source_interaction)



func _normalize_twooter_liked_post(source_like: Variant) -> Dictionary:
	return TwooterStateSystem.normalize_liked_post(source_like)



func _normalize_twooter_message_thread(source_thread: Variant) -> Dictionary:
	return TwooterStateSystem.normalize_message_thread(source_thread)



func _normalize_twooter_dialog_branch(source_branch: Variant) -> Dictionary:
	return TwooterStateSystem.normalize_dialog_branch(source_branch)
...
func _normalize_thesis_research_tray(source_tray: Variant) -> Dictionary:
	var normalized: Dictionary = {}
	if typeof(source_tray) == TYPE_ARRAY:
		for row_value in source_tray:
			if typeof(row_value) != TYPE_DICTIONARY:
				continue
			var row: Dictionary = _normalize_research_evidence_row(row_value)
			var row_id: String = str(row.get("id", ""))
			if not row_id.is_empty():
				normalized[row_id] = row
		return normalized
	if typeof(source_tray) != TYPE_DICTIONARY:
		return normalized
	var source: Dictionary = source_tray
	for row_id_value in source.keys():
		if typeof(source.get(row_id_value)) != TYPE_DICTIONARY:
			continue
		var row: Dictionary = _normalize_research_evidence_row(source.get(row_id_value, {}))
		var row_id: String = str(row.get("id", ""))
		if row_id.is_empty():
...
func _normalize_research_evidence_row(source_row: Variant) -> Dictionary:
	var source: Dictionary = source_row if typeof(source_row) == TYPE_DICTIONARY else {}
	var row_id: String = str(source.get("id", ""))
	var category: String = str(source.get("category", ""))
	var interpretation: String = str(source.get("interpretation", ""))
	var normalized: Dictionary = {
		"id": row_id,
		"company_id": str(source.get("company_id", "")),
		"ticker": str(source.get("ticker", "")),
		"company_name": str(source.get("company_name", "")),
		"sector_id": str(source.get("sector_id", "")),
		"sector_name": str(source.get("sector_name", "")),
		"category": category,
		"category_label": str(source.get("category_label", category.capitalize())),
		...
		"impact": str(source.get("impact", "mixed")),
		"captured_day_index": int(source.get("captured_day_index", source.get("day_index", day_index))),
	...
	if source.has("player_note"):
		normalized["player_note"] = str(source.get("player_note", ""))
	if source.has("source_evidence_id"):
		normalized["source_evidence_id"] = str(source.get("source_evidence_id", ""))
	if typeof(source.get("captured_trade_date", {})) == TYPE_DICTIONARY:
...
func _apply_corporate_action_applications(applications: Array) -> void:
	...
	for application_value in applications:
		...
		match str(application.get("application_type", "")):
			"rights_issue":
				CorporateActionApplications.apply_rights_issue_application(self, application)
			"private_placement":
				CorporateActionApplications.apply_private_placement_application(self, application)
			"restructuring":
				CorporateActionApplications.apply_restructuring_application(self, application)
			"stock_buyback":
				CorporateActionApplications.apply_stock_buyback_application(self, application)
			"stock_split":
				CorporateActionApplications.apply_stock_split_application(self, application)
			"tender_offer":
				CorporateActionApplications.apply_tender_offer_application(self, application)
			"strategic_merger_acquisition":
				CorporateActionApplications.apply_strategic_mna_application(self, application)
			"backdoor_listing":
				CorporateActionApplications.apply_backdoor_listing_application(self, application)
			"ceo_change":
				CorporateActionApplications.apply_ceo_change_application(self, application)
			"backdoor_lockup_update":
				CorporateActionApplications.apply_backdoor_lockup_update_application(self, application)
			"backdoor_milestone_update":
				CorporateActionApplications.apply_backdoor_milestone_update_application(self, application)
...
func _build_quarterly_filing_payload(report: Dictionary, trading_day_number: int, trade_date: Dictionary, macro_state: Dictionary = {}) -> Dictionary:
	...
	var qoq_growth_target: float = clamp(
		...
		(macro_pressure * FILING_QOQ_MACRO_WEIGHT) +
		(micro_pressure * FILING_QOQ_MICRO_WEIGHT) +
		(market_pressure * FILING_QOQ_MARKET_WEIGHT) +
	...
	var yoy_growth_target: float = clamp(
		base_revenue_growth +
		(macro_pressure * FILING_YOY_MACRO_WEIGHT) +
		(micro_pressure * FILING_YOY_MICRO_WEIGHT) +
		(market_pressure * FILING_YOY_MARKET_WEIGHT) +
	...
	var quarter_revenue: float = max(lerpf(revenue_from_qoq, revenue_from_yoy, FILING_YOY_BLEND_WEIGHT), 1.0)
	...
	var margin_pressure: float = (
		((float(traits.get("margin_strength", 0.5)) - 0.5) * FILING_MARGIN_STRENGTH_WEIGHT) +
		((float(traits.get("execution_consistency", 0.5)) - 0.5) * FILING_EXECUTION_CONSISTENCY_WEIGHT) +
		((float(traits.get("balance_sheet_strength", 0.5)) - 0.5) * FILING_BALANCE_SHEET_WEIGHT) -
